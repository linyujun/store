 ...
 */
package com.kinzhan.dev.platform.util.filter;
...
public class QueryHelpPlus {
    ...
    public static <R, Q> QueryWrapper getPredicate(R obj, Q query, QueryWrapper defaultWrapper) {

        QueryWrapper<R> queryWrapper = Optional.ofNullable(defaultWrapper).orElse(new QueryWrapper());
        if (query == null) {
            return queryWrapper;
        }
        try {
                    ...
                    switch (q.type()) {
                        ...
                        case BETWEEN:
                           ...
                           queryWrapper.between(finalAttributeName, between.get(0), between.get(1));
                            break;
                        case APPLY_SQL:
                            queryWrapper.apply(String.format(q.template(), finalAttributeName, val));
                            break;
                        case UNIX_TIMESTAMP:
